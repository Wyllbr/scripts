...
HOSTGROUP_ID=(49 50 51 52 53 60)  # ID do grupo de hosts no Zabbix 
...
get_zabbix_hosts_by_group() {
	...
	for hostgroupid in $HOSTGROUP_ID; do
...
#Funcao para obter o id do phpipam da subrede que o endereco passado e>
get_subnet_id() {
    local NETWORK="$1"
...
change_mac_format() {
    local MAC="$1"
    echo "$MAC" | sed -E 's/\b([0-9A-Fa-f])\b/0\1/g'
}
